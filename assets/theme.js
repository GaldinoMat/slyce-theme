...
(function(){
  ...
  //$(document).ready(function() {
    // Selecione o link "Feminino" usando uma classe específica correta
    // Certifique-se de substituir "sua-classe-especifica" pela classe real do link "Feminino"
    //$('.grid__item.medium-up--one-fifth.appear-animation.appear-delay-4').find('a').removeAttr('href');});
      ...
      colorSwatchHovering: function() {
        ...
        this.container.querySelectorAll(selectors.colorSwatch).forEach(swatch => {
          swatch.addEventListener('mouseenter', () => {
            var id = swatch.dataset.variantId;
            var image = swatch.dataset.variantImage;
            const elParent = swatch.parentElement.parentElement
            const queryString = '.grid-product__color-image--' + id
            const el = elParent.querySelector(queryString);
          ...
          swatch.addEventListener('mouseleave', () => {
            var id = swatch.dataset.variantId;
            const elParent = swatch.parentElement.parentElement
            elParent.querySelector('.grid-product__color-image--' + id).classList.remove('is-active');
          });
        });
      },
...
document.addEventListener("DOMContentLoaded", function() {
    // Seleciona todos os elementos que podem conter o texto ",00"
    var elementos = document.querySelectorAll("*");

    // Itera sobre cada elemento
    elementos.forEach(function(elemento) {
        // Verifica se o elemento contém texto
        if (elemento.childNodes.length > 0) {
            elemento.childNodes.forEach(function(child) {
                // Se for um nó de texto, substitui ",00" por ""
                if (child.nodeType === Node.TEXT_NODE) {
                    child.nodeValue = child.nodeValue.replace(/,00/g, '');
                }
            });
        }
    });
